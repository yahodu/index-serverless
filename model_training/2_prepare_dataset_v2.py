...
class MXRecordDatasetReader:
    ...
    _MAGIC     = 0xced7230a
    _JPEG_SKIP = 16  # confirmed by probe_rec.py
    ...
    def _read_record_at(self, offset: int) -> bytes:
        ...
        self._rec_file.read(16)  # skip index + id header
    ...
    def read_image(self, index: int) -> np.ndarray:
        """Read image at position `index`. Returns (112, 112, 3) RGB uint8."""
        key    = self.keys[index]
...
class FolderDatasetReader:
    ...
    Reads face images from a folder structure:
      dataset_root/
        ...
        person_002/
          ...
    Images should ideally be pre-aligned to 112x112. If not, they will be resized.
    """
...
class TeacherEmbeddingGenerator:
    ...
    chunk_size guidance:
      Smaller chunks  = more frequent saves, less RAM, faster I/O per save
      Larger chunks   = fewer files, slightly more RAM usage
      Recommended     = 10_000 (avoids the savez_compressed CPU stall)
    """
    ...
    def __init__(
        ...
        sess_options.log_severity_level   = 3  # suppress shape-mismatch warnings
    ...
    def preprocess(self, images: np.ndarray) -> np.ndarray:
        ...
        Output: (N, 3, 112, 112) float32 in [-1, 1]
        """
    ...
    @staticmethod
    def _save_chunk(chunk_file: str, images: np.ndarray, embeddings: np.ndarray):
        """
        Save a chunk to disk.

        We use np.savez (uncompressed) instead of np.savez_compressed because:
          - Images are decoded JPEGs — already close to their entropy limit
          - Compressing 100k × 112 × 112 × 3 bytes stalls the CPU for minutes
          - Embeddings are float32 and compress poorly anyway
          - Uncompressed .npz loads ~10x faster during training too
        """
        np.savez(chunk_file, images=images, embeddings=embeddings)
    ...
    def process_dataset_rec(
        ...
        chunk_size:  int = 10_000,   # reduced from 100_000 — see class docstring
    ):
        """
        ...
        for start in range(0, total, self.batch_size):
            ...
            # Flush to disk when chunk is full or we've reached the end
            if processed >= chunk_size or end >= total:
                ...
                self._save_chunk(chunk_file, all_imgs, all_embs)
                print(f"\nSaved {chunk_file}: {all_imgs.shape[0]} samples "
                      f"| images {all_imgs.nbytes / 1e6:.0f} MB "
                      f"| embeddings {all_embs.nbytes / 1e6:.0f} MB")
...
def sanity_check_model(teacher: TeacherEmbeddingGenerator, n_images: int = 2) -> bool:
    ...
    try:
        ...
        assert embs.shape == (n_images, 512), f"Unexpected shape: {embs.shape}"
...
if __name__ == "__main__":
    ...
    teacher = TeacherEmbeddingGenerator(
        ...
        batch_size=24,         # adjust to your GPU — see class docstring
        gpu_mem_limit_gb=12.0,  # adjust to your GPU
    )

    reader_ok = sanity_check_reader(REC_PATH, IDX_PATH)
    if not reader_ok:
        print("\nERROR: RecordIO reader failed.")
        sys.exit(1)
    ...
    teacher.process_dataset_rec(
        ...
        chunk_size=10_000,     # small chunks = fast saves, no CPU stall
    )
